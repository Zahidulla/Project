...
Login
    ...
    Sleep    2s
    ${PasswordFieldStatus}      Run Keyword And Return Status    Element Should Be Visible    ${PasswordField}
    FOR    ${i}    IN RANGE   3
        IF     ${PasswordFieldStatus} == ${False}
            Press Keys      ${EmailInputField}      RETURN
            Sleep    2s
            ${PasswordFieldStatus}      Run Keyword And Return Status    Element Should Be Visible    ${PasswordField}
        END
    END
